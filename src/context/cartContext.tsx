...
import { CartContextType, Dish, Menu, CartState, DishCartContext } from "../types/type";
...
const cartReducer = (state: CartState, action: CartAction): CartState => {
    switch (action.type) {
        ...
        case "ADD_DISH":
            ...
            const existingDishIndex = state.dishes.findIndex(d => d.dish.id === action.payload.id);
            ...
            if (existingDishIndex !== -1) {
                ...
                updatedDishes = [...state.dishes, { dish: action.payload, quantity: 1, nbElement: 1 }];
            }
            ...
            case "REMOVE_DISH": {
                ...
                const newTotalRemoveDish = parseFloat((state.total - removedDish.dish.price).toFixed(2));
                let updatedDishes;
                if (removedDish.quantity > 1) {
                    // Si la quantité est > 1, on décrémente seulement
                    updatedDishes = state.dishes.map((d, index) =>
                        index === dishIndex ? { ...d, quantity: d.quantity - 1, nbElement: d.nbElement - 1 } : d
                    );
                } else {
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
    ...
    return (
        <CartContext.Provider value={{ ...state, addComment, addDish, removeDish, addMenu, removeMenu }}>
            {children}
