...
import { CodeSchema, LoginFormSchema, SendCodeSchema } from "../../types/type";
...
export const CodeConfirm = () => {
    ...
    const zodFormSchema: ZodType<CodeSchema> = z.object({
        code: z.number().min(6, { message: "Le code doit être d'au moins 6 caractères" }),
    });
    ...
    } = useForm<CodeSchema>({ resolver: zodResolver(zodFormSchema) });

    const handleSubmitForm = async (data: SendCodeSchema) => {
        setCodeError(null); // Réinitialiser l'erreur à chaque tentative

        try {
            const response = await fetch(`${api_url}/auth/reset-password-code-confirm`, {
                method: "POST",
                headers: {
            ...
            window.location.replace(front_url + "/sendCode/codeConfirm");
        } catch (error: any) {
            setCodeError(error.message); // Afficher l'erreur sous le bouton
        }
    };
